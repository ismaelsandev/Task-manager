<?php
    ...
    $stmt = $conn->prepare("SELECT id FROM dashboard WHERE user_id = ?");
    ...
    if ($stmt->num_rows > 0) {
        ...
        // Insertar
        echo "Insertar";
        $nombre = "Mi primer dashboard";
        $stmt = $conn->prepare("INSERT INTO dashboard (user_id, nombre, contenido) VALUES (?, ?, ?)");
        $stmt->bind_param("iss", $user_id, $nombre, $contenido);
    }
    $stmt->execute();
    $stmt->close();
?>